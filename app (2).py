...
@st.cache_resource
def load_model():
    # Try the expected path, then a couple of common fallback locations,
    # in case the file was placed in a subfolder during deployment.
    ...
    # Last resort: pick up any .pkl file sitting next to app.py.
    if here.exists():
    ...
    # Nothing found — show a clear, actionable error instead of a raw traceback.
    ...
    st.error(
        ...
        "folder as `app.py` (check it wasn't skipped by `.gitignore` or a "
        "GitHub file-size limit — files over 100MB need Git LFS)."
    )
    st.stop()
